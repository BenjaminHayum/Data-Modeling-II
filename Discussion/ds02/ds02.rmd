...
You can do the exercises below individually, but we recommend you work on them in a small group. **Choose 2** of the following exercises to complete.
...
```{r}
robbin_constant = function(n) {
  distances = seq(1, n)
  for (i in 1:n) {
    coords = runif(6)
    x_dist = coords[4] - coords[1]
    y_dist = coords[5] - coords[2]
    z_dist = coords[6] - coords[3]
    distances[i] = sqrt(x_dist^2 + y_dist^2 + z_dist^2)
  }
  
  return(mean(distances))
}

value = robbin_constant(1000000)
```
...
percent_error = ((value - actual)/actual)*100
percent_error
```
...
```{r}
monte_carlo_loop = function(n, m) {
  head_runs = c()
  for (i in 1:m) {
    flips = rbinom(n = n, size = 1, p = 0.5)
    runs = longest_head_run(flips)
    head_runs[i] = runs
  }

  return(mean(head_runs))
}

monte_carlo_loop(30, 10000)
```

```{r}
monte_carlo_vector = function(n, m) {
  flips - rbinom(n = n*m, size = 1, p = 0.6)
  flips_mat = matrix(data = flips, ncol = n, byrow=TRUE)
  head_runs = apply(flips_mat, Margin = 1, FUN = longest_head_run())
