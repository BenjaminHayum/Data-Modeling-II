...
This should also prove useful in completing HW5!
...
Because this is the odds of dying from lightning independent of anything else. However, in this case, you can't be independent from the fact that you are in a lightning storm and it just struck right near you. Given this situation, your odds are way higher than 1 in 7,000,000
...
We'll have lots more to say about that in Thursday's lecture, but let's think today about how we might go about estimating a probability like this.
...
rmymixture <- function(n) {
  z = rbinom(n = n, size = 1, p = 0.5)
  x = rep(0, n)
  for (i in 1:n) {
    if (z[i] == 1) {
      x[i] = rnorm(1, mean = 2, sd = 1)
    } else {
      x[i] = rnorm(1, mean = -2, sd = 1)
    }
  }
...
```{r}
rvs = rmymixture2(2000)
hist(rvs %>% filter(z==1) %>% pull(x))
```


```{r}
rvs = rmymixture2(2000)
hist(rvs %>% filter(z==0) %>% pull(x))
```
------------------------------------------------------------------------

Wah Wah wah, bitch, I am in fact lil baby...
