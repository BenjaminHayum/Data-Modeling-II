...
Import it normally using `read_csv`.
...
# 
library(tidyverse)
ratings.standard = read_csv("../../Data/chess.csv")
...
ratings.standard.in = ratings.standard %>% 
   filter(Country == "India" & B_day >= 2000)
...
There are two dice, one 40-sided, and one 50-sided. What is the probability that the 50 sided dice produces a larger value on a given roll?
```{r}
NMC = 100000
fifty_greater_counter = 0
for (i in 1:NMC) {
  fifty_roll = sample(x=50, size=1)
  forty_roll = sample(x=40, size=1)
  if (fifty_roll > forty_roll) {
    fifty_greater_counter = fifty_greater_counter + 1
  }
}
fifty_greater_counter/NMC
```
