...
Many of the coefficients fully go to 0 and only a 3 remain plus the intercept.
...
```{r}
coef(glmnet(x_mtc, y_mtc, alpha = 1, lambda=1:6))
```

***

At a lambda value of 6, all of the coefficients go to 0 besides the intercept
...
A lambda of 5 as shown above OR use a sequence with smaller value jumps of lambda above
