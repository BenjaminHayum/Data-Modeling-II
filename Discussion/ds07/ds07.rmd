...
resample_and_estimate <- function(phat, nsamp) {
  ...
  data_star = rbinom(n = 1, size = nsamp, prob = phat)
...
for (i in 1:Nrep) {
  replicates[i] <- resample_and_estimate(phat = phat, nsamp = n) # TODO: fix this to store a single replicate!
}
```
...
```{r}
# 90% Simulated Confidence Interval!!
...
quantile(replicates, probs = c(lowerBound, upperBound))
...
for (i in 1:Nexpt) {
  data = rbinom(n = 1, size = n, prob = ptrue)
  phat = compute_phat(data)
  # TODO: Call a function here that computes a CI, like run_trial from lecture
  B = 500
  replicates = rep(NA, B)
  for (j in 1:B) {
    replicates[j] <- resample_and_estimate(phat = phat, nsamp = n)
  }
  bounds = quantile(replicates, probs = c(lowerBound, upperBound))
  if (ptrue > unname(bounds[1]) && ptrue < unname(bounds[2])) {
    ptrue_in_CI[i] <- TRUE # TODO: update this to be a Boolean that says whether or not ptrue was in the CI on this trial.
  } else {
    ptrue_in_CI[i] <- FALSE
  }
}
...
```{r}
alpha = .1
z_star = abs(qnorm(alpha/2, mean = 0, sd = 1))
upperBound = xbar + z_star*sqrt(varhat/n)
lowerBound = xbar - z_star*sqrt(varhat/n)
