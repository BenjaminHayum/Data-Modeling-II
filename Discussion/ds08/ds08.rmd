...
As usual, we encourage you to work in small groups, but this is up to you.
...
ggplot(quakes.count, aes(x = mag.binned, y = log10(N))) + 
  geom_point() + geom_line(aes(linetype = year, color = year)) + 
  xlab("M") + 
  ylab("log10(N)") + 
  ggtitle("Don't know what's going on...")
```
...
```{r}
#TODO: uncomment and fill in the code below.
lm.quakes <- lm(formula = log10(N) ~ 1 + mag.binned, data = quakes.count);
```
...
If we followed the same linear regression procedure on data from experiments similarly constructed, in 95% of them we would expect to find the true intercept/Beta0 and the true slope/Beta1.
