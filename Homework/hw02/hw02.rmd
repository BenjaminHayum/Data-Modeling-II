...
birthday <- function(n, k, m = 1000) {
  k_num_shared_bday_counter = 0
  for (i in 1:m) {
    n_birthdays = sample(x = 365, size = n, replace = TRUE)
    if (max(table(n_birthdays)) >= k) {
      k_num_shared_bday_counter = k_num_shared_bday_counter + 1
    }
  }
  
  return(k_num_shared_bday_counter/m)
}
...
The Monte Carlo estimated probability that at least 5 students in our 241 student class have the same birthday is about .19
...
rx <- function(n) {
  curr_input = runif(n=n)
  return( sqrt(curr_input) )
}
...
```{r}
# 1-7 are the warmest 7 years
warmest_seven_years_straight = function(n){
  seven_warmest_straight_counter = 0
  for (i in 1:n) {
    year_warmth_order = sample(10, size = 10, replace = FALSE)
    if ( sum(1:7) == sum(tail(year_warmth_order, n = 7)) ) {
      seven_warmest_straight_counter = seven_warmest_straight_counter + 1
    }
  }
  return(seven_warmest_straight_counter/n)
}

warmest_seven_years_straight(10000)
```
