...
The gallant estimate is very accurate and close to the true value because it calculated from the same process as the equation dictates. The goofus estimate, however, is further from the true value because it doesn't take into account the 2Cov(Wp, Mp) factor in the equation.
...
`generate_correlated_sample(n,c)` will generate a vector of `n` observations in which each entry is a normal with mean 0 and variance 1, and consecutive pairs of entries in the vector have covariance `c`.
...
run_correlated_test <- function(n, c) {
  X = generate_correlated_sample(n, c)
  Y = generate_correlated_sample(n, c)
...
estimate_level <- function(c) {
  NMC = 10000
  n = 20
  significance_check = rep(NA, NMC)
  for (i in 1:NMC) {
    p_value = run_correlated_test(n, c)
    if (p_value <= .05) {
      significance_check[i] = TRUE
    } else { 
      significance_check[i] = FALSE
    }
  }
  
  return(sum(significance_check)/NMC)
}
```
...
```
```{r}
inputs = seq(-0.2, 0.2, by=.05)
outputs = rep(NA, length(inputs))
for (i in 1:(length(inputs))) {
  curr_input = inputs[i]
  outputs[i] = estimate_level(curr_input)
}

df = data.frame(inputs = inputs, outputs = outputs)
ggplot(data = df) + 
  geom_line(aes(x = inputs, y = outputs), color = "black") + 
...
When the covariance is negative, we are below the alpha level. When the covariance is around 0, we are about at the alpha level. When the covariance is positive, we are above the alpha level.
...
pr_disease_given_pos <- function(p_dis, sensitivity, p_pos_given_nodis) {
  p_pos = pr_positive_test(p_dis, sensitivity, p_pos_given_nodis)
  return((sensitivity*p_dis)/p_pos)
}
```
...
```{r}
p_disease_inputs = 10^(-8:-1)
p_disease_given_pos_test = rep(NA, length(p_disease_inputs))
for (i in 1:length(p_disease_inputs)) {
  curr_input = p_disease_inputs[i]
  p_disease_given_pos_test[i] = pr_disease_given_pos(curr_input, .9999, 10**-3)
}
df = data.frame(p_disease_inputs = p_disease_inputs, p_disease_given_pos_test =  p_disease_given_pos_test)

ggplot(data = df) + 
  geom_line(aes(x = p_disease_inputs, y = p_disease_given_pos_test), color = "black") +
...
Let's use 7 cases per 100,000 as our estimate of $\Pr[ \text{ disease } ]$.
...
```{r}
p_pos = 158/4049
p_disease_given_pos = 116/158
p_disease = 7/100000
p_positive_given_disease = (p_disease_given_pos*p_pos)/p_disease
p_positive_given_disease

#Not from 0 to 1..but TA at office hours said this is okay
```
