...
p = 0.82
NMC = 100000
batch_test_sizes = 1:20

proportion_over_five_functional = rep(NA, length(batch_test_sizes))
for (i in 1:length(batch_test_sizes)) {
  curr_batch_size = batch_test_sizes[i]
  num_functional_widgets = rbinom(n = NMC, size = curr_batch_size, prob = p)
  proportion_over_five_functional[i] = mean(num_functional_widgets >= 5) 
}
df = data.frame(batch_test_sizes = batch_test_sizes, proportion_over_five_functional = proportion_over_five_functional)
df
```

***

For each possible batch size from 1 to 20 (an upper bound chosen post-hoc after having outputted multiple data frames and seeing around where the proportion crosses over .99), I found the number of functional widgets that would come to be given that batch size as well the probability that a widget is functional of .82 using 100,000 Monte Carlo runs of a binomial distribution. I then found the proportion of these numbers that were greater or equal to 5 functional widgets and outputted a data frame mapping on the batch size to this proportion. From this, I found that a batch size of 10 is the minimum that is sufficient to have a probability of over .99 that there will be over 5 functional widgets.
...
There appears to be a downward trend where the higher the horsepower gets, the lower the miles per gallons gets. Therefore, it seems like the two variables are dependent. If they were independent, there'd be a slope of about 0 with no visible trend on the plot.
...
```{r}
observed_test_stat = cor(hp, mpg, method = "pearson")

permute_and_ONE_test_stat = function(hp, mpg) {
  n_hp = length(hp)
  n_mpg = length(mpg)
  
  shuffled_hp = sample(hp, size = n_hp, replace = FALSE)
  shuffled_mpg = sample(mpg, size = n_mpg, replace = FALSE)

  return(cor(shuffled_hp, shuffled_mpg, method = "pearson"))
}

NMC = 10000
null_test_stats = rep(NA, NMC)
for (i in 1:NMC) {
  null_test_stats[i] = permute_and_ONE_test_stat(hp, mpg)
}

( sum(null_test_stats <= -abs(observed_test_stat)) + sum(null_test_stats >= abs(observed_test_stat)) ) / NMC
```

***

I wrote a function to permute and calculate one null test statistic of pearson correlation. I then called this function with 10000 Monte Carlo simulations to calculate 10000 test statistics. I then compared these to the actual observed test statistic and found how many null test statistics were at or more extreme than the observed test statistic and divided this number by the amount of Monte Carlo simulations. This found the approximated p-value of 0 -- which means that i.e. of all the 10000 simulated null test statistics 0 of them were at or more extreme than the observed test statistic. 0 is less than any significance level you can choose. Therefore, this result is statistically significant and there is strong evidence to reject that horsepower and miles per gallon are not correlated.
...
sigma2hat <- function( data ) {
  ...
  for (i in 1:n) {
    sum_squared_distances = sum_squared_distances + (data[i] - muhat)^2
  }
  return(sum_squared_distances/n)
}
```
...
run_trial <- function( nsamp ) {
  ...
  estimated_sigma2 = sigma2hat(data)
  ...
  return(estimated_sigma2 - 1)

}

#Return actual variance of data? Or expected variance of 1 from a standard normal distribution?
```
...
estimate_bias <- function( nsamp, NMC ) {
  ...
  MC_replicates = rep(NA, NMC)
  for (i in 1:NMC) {
    MC_replicates[i] = run_trial(nsamp)
  }
  
  return(mean(MC_replicates))
}
```
...
```{r}
estimate_bias(nsamp = 20, NMC = 2000)
```
...
estimate_bias_adjusted <- function( nsamp, NMC ) {
  ...
  MC_replicates = rep(NA, NMC)
  for (i in 1:NMC) {
    MC_replicates[i] = run_trial_adjusted(nsamp)
  }
  
  return(mean(MC_replicates))
}

estimate_bias_adjusted(nsamp = 20, NMC = 2000)
```
